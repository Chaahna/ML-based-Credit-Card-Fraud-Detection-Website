...
def root():
    if 'email' in session:
        return render_template_string("<h1>You are logged in</h1> </h1><a href=\"/index\">Get Started</a> </h1><a href=\"/changePassword\">change pass</a> <a href=\"/logout\">logout</a>")
    else:
...
def results():
    if request.method == "POST":
        ...
        print('Transcation Amount is {0}'.format(amount))
        ...
        new_input_tuple['Amount'] = amount
        new_input_tuple['Time'] = current_time
        ...
        return render_template("results.html", lr_prediction=lr_prediction, rf_prediction=rf_prediction,y_true=y_true, input_tuple=new_input_tuple, time=current_time, amount=amount)
